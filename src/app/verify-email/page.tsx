...
} from "@/app/actions/authActions"; // Naya action import kiya gaya hai
      ...
      <div className="w-full max-w-md p-8 sm:p-10 space-y-6 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700">
          ...
          <div className="flex justify-center gap-2 md:gap-3">
            {otp.map((digit, index) => (
              <input
                ...
                className="w-12 h-14 md:w-14 md:h-16 text-center text-2xl font-semibold border-2 border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 focus:border-brand-primary focus:ring-1 focus:ring-brand-primary transition"
                disabled={isLoading}
              />
            ))}
          </div>
        ...
        <p className="text-center text-sm text-gray-500 dark:text-gray-400">
          ...
          <button
            ...
            className="font-medium text-brand-primary hover:underline disabled:text-gray-400 dark:disabled:text-gray-500 disabled:cursor-not-allowed transition-colors"
          >
            {isResending
              ? "Sending..."
              : cooldown > 0
