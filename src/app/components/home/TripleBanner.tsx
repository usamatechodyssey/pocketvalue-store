// components/TripleBanner.tsx - CORRECTED & FINAL
...
export default function TripleBanner({ banners }: StoryBannersProps) {
  ...
  // --- FIX IS HERE: Andar wala 'div' hata diya gaya hai ---
  // Ab yeh component page.tsx se anay wali padding ko istemal karega.
  return (
    <section>
        ...
        {banners.map((banner) => (
          ...
          >
            <Image
              ...
              width={600}
              height={400}
              className="w-full h-auto aspect-4/3 object-cover transition-transform duration-500 ease-in-out group-hover:scale-110" // h-[40] ki jagah aspect ratio use karein
              priority
            />
...
// --- SUMMARY OF CHANGES ---
// - Removed Wrapper Div: Maine woh `div` hata diya hai jis par `max-w-[1240px] mx-auto px-4` laga hua tha. Ab yeh component seedha `section` se shuru hota hai.
// - Removed Section Padding: Maine `<section>` tag se `pt-8 pb-6` classes bhi hata di hain, kyunke ab yeh spacing `page.tsx` me `space-y-*` class se control ho rahi hai.
// - Fixed Image Height: Maine `h-[40]` class ko `h-auto aspect-[4/3]` se badal diya hai. Is se image hamesha sahi proportion me nazar aayegi aur stretch nahi hogi.
