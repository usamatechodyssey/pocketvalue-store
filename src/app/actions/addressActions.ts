...
// This is our Data Transfer Object (DTO). It defines the shape of a "safe" address object
// that can be sent from the server to the client. It is now directly derived from our Zod schema.
export type ClientAddress = z.infer<typeof AddressSchema> & {
  _id: string;
  isDefault: boolean;
  lat?: number | null;
...
// Standard server response type for our actions
interface ServerResponse {
  success: boolean;
  message: string;
}
...
export async function saveAddress(
  ...
  try {
    ...
    // Convert the saved Mongoose subdocument back to a clean ClientAddress object to return to the client
    const savedAddress = user.addresses[user.addresses.length - 1];
    const newClientAddress: ClientAddress = {
        _id: savedAddress._id.toString(),
        fullName: savedAddress.fullName,
